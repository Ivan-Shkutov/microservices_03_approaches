...
- Сбор метрик:
  ...
  1. На каждом хосте установлен Node Exporter, собирающий системные метрики (CPU, RAM, HDD, Network).

  2. Для каждого контейнера используется cAdvisor, который собирает метрики по потреблению ресурсов на уровне сервисов.

  3. Для специфичных метрик сервиса используются Custom Exporters (можно встроить Prometheus client library).

- Хранение и запрос метрик:

  1. Prometheus опрашивает все экспортеры (Node Exporter, cAdvisor, Custom Exporters) через HTTP endpoints.

  2. Метрики хранятся в формате time-series с временной меткой.

  3. Используется язык запросов PromQL для фильтрации, агрегации и создания алертов.

- Визуализация и интерфейс:

  1. Grafana подключается к Prometheus и предоставляет интерфейс:

      - Создание кастомных дашбордов для хостов, сервисов и специфичных метрик.

      - Группировка, агрегирование и фильтрация метрик.

      - Настройка панелей с графиками, гистограммами, alert-листами.

  2. Возможность предоставлять доступ разработчикам с разными уровнями прав.
     

![3](https://github.com/Ivan-Shkutov/microservices_03_approaches/blob/main/3.png)
